...
```{r}
avg_arrival_delay <- my_flights %>% group_by(dest) %>% summarise(avg_arrival_delay = mean(arr_delay))
...
```{r}
 avg_arrival_delay %>% slice_max(avg_arrival_delay, n = 10) %>% select(dest, avg_arrival_delay)
...
airlines
avg_arrival_delay
full_join(avg_arrival_delay, airlines)
